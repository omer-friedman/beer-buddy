...
const BeerCard = (props) =>{
    const {product} = props
    const [isCardExpand, setExpand] = useState(false)
    ...
    const maskTransitions = useTransition(isCardExpand, null, {
        from: { opacity: 0 },
        enter: { opacity: 1 },
        leave: { opacity: 0 },
    })

    const largeCardTransition = useTransition(isCardExpand, null, {
            ...
            <CardBox islarge={isLarge}>
                ...
                {!isLarge && 
                    <RowContainer>
                        ...
                        <ExpandButton expandBtnClicked={() => setExpand(!isCardExpand)} />
                        {
                            routePath.toString() === '/favorites' &&
                            <BeerRank id={product.id}/>
                        }   
                    </RowContainer>
                }
                {
